...
const aggregateDataByTeam = (tickets: Ticket[]) => {
  ...
  Object.keys(teamSummary).forEach((team) => {
    ...
    teamData.totalTimeToResolve =
      teamData.totalTimeToResolve / teamData.totalTickets;
    teamData.totalCustomerSatisfaction =
...
const generateHtmlTable = (teamSummary: {
  ...
  Object.keys(teamSummary).forEach((team) => {
    ...
    tableHtml += `<tr>
                        ...
                        <td>${(teamData.totalTimeToResolve / 60).toFixed(
                          2
                        )}</td> <!-- Convert minutes to hours -->
                        <td>${teamData.totalCustomerSatisfaction.toFixed(
                          2
                        )}</td>
                      </tr>`;
  });
